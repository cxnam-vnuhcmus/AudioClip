...
from model_phoneme.visual_encoder import VisualEncoder, BinaryImageFeatureExtractor
from model_phoneme.text_encoder import TextEncoder
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        # )
        self.visual = BinaryImageFeatureExtractor()
    ...
    def encode_text(self, text: List[str]) -> torch.Tensor:
        ...
        for i in range(len(text)):
            text_joined = [', '.join(entities) for entities in text[i]]
            text_tokens = torch.cat([
                tokenize(base_str.format(entities)) for entities in text_joined
            ])
            text_tokens = text_tokens.to(self.device)
            txt_tokens.append(text_tokens)
        text_tokens = torch.stack(txt_tokens)
    ...
    def forward(self,
        ...
        if image is not None:
            image_features = self.encode_visual(image)
    ...
    def eval_step_imp(model, batch, device) -> Tuple[torch.Tensor, torch.Tensor]:
        with torch.no_grad():
            ...
            ((audio_features, image_features, text_features), (logits_audio_image, _, _)), _ = model(
                audio=audio,
                image=image,
                text=text,
                batch_indices=torch.arange(audio.shape[0], dtype=torch.int64, device=device)
