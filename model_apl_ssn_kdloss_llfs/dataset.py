...
from .utils import FACEMESH_ROI_IDX, extract_llf_features, convert_emotion_to_one_hot
...
class Dataset(td.Dataset):
    ...
    def __getitem__(self, idx):        
        ...
        return (mel_segment, llfs_segment, lm_data_list, emo_tensor, os.path.join(lm_folder,lm_paths[segment_start_idx + (self.n_frames + 1)//2-1]))

    def collate_fn(self, batch):
        batch_audio, batch_llfs, batch_landmark, batch_emo, lm_paths = zip(*batch)
        ...
        if not all(img is None for img in batch_emo):
            batch_emo = [batch_emo[idx] for idx in keep_ids]
            batch_emo = torch.stack(batch_emo, dim=0)
        else:
            batch_emo = None
        ...
        return batch_audio, batch_llfs, batch_landmark, batch_emo, lm_paths
