...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.audio_emotion_classifier = EmotionClassifier(input_size=128, hidden_size=64, num_classes=8)
        self.lm_emotion_classifier = EmotionClassifier(input_size=128, hidden_size=64, num_classes=8)
    ...
    def forward(self,
        ...
        loss = 2* lm_loss + 1.5 * kd_loss + audio_ce_loss + lm_ce_loss
    ...
    def inference(self, batch, device, save_folder):
        with torch.no_grad():
            audio, landmark, gt_emo, lm_paths = batch
            seg_len = (landmark.shape[1] + 1)//2
