...
def convert_emotion_to_one_hot(emotion_label):
    num_classes = len(emotion_to_idx)
    emotion_idx = emotion_to_idx[emotion_label]
    one_hot_vector = torch.zeros(num_classes)  # Khởi tạo vector one-hot toàn 0
    one_hot_vector[emotion_idx] = 1  # Đặt vị trí tương ứng bằng 1
    return one_hot_vector