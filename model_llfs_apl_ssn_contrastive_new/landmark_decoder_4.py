...
class LandmarkDecoder(nn.Module):
    def __init__(self, input_dim=128, hidden_dim=128, output_dim=131*2):
        ...
        self.linear1 = nn.Linear(input_dim*2 + 32, hidden_dim)
        self.linear2 = nn.Linear(hidden_dim, output_dim )
        self.shortcut1 = nn.Linear(input_dim*2 + 32, output_dim)
        ...
        self.gate = nn.Sequential(
            ...
            nn.Sigmoid()          # Activation to control the gate
        )

        

    def forward(self, audio_feature, llfs_feature, lm_feature):
        combined_feature = torch.cat((audio_feature, llfs_feature, lm_feature), dim=-1)  # (B, N, 128)
        ...
        # gated_combined = y * gate_value
        
        # output, _ = self.mha(x, gated_combined, gated_combined)  # (q, k, v)
