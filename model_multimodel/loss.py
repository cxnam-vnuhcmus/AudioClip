...
def cosine_similarity_loss(gt, pred):
    """
    Tính cosine similarity loss giữa ground truth (gt) và predicted (pred) landmarks.
    gt: Tensor kích thước [B, N, 131, 2]
    pred: Tensor kích thước [B, N, 131, 2]
    """
    # Đưa tensor về dạng [B, N, 262] (kết hợp các tọa độ 2D thành 1 vector)
    gt_flat = gt.view(gt.size(0), gt.size(1), -1)  # [B, N, 262]
    pred_flat = pred.view(pred.size(0), pred.size(1), -1)  # [B, N, 262]

    # Tính cosine similarity giữa gt và pred
    cos_sim = F.cosine_similarity(gt_flat, pred_flat, dim=-1)
    
    # Cosine similarity loss là 1 - cosine similarity
    cos_sim_loss = 1 - cos_sim
    
    # Tính trung bình loss cho toàn bộ batch
...
class CustomLoss(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        # cs_loss = cosine_similarity_loss(target, pred)
